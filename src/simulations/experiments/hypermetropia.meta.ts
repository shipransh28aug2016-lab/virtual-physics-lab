// GENERATED FILE — do not edit by hand.
// Regenerate with: node scripts/gen-catalogue.mjs
import type { ExperimentMeta } from '@/experiments/registry';

export const meta: ExperimentMeta = {
  id: 'hypermetropia', slug: 'hypermetropia',
  title: 'Hypermetropia (Long-Sightedness) and its Correction',
  shortTitle: 'Hypermetropia',
  aim: 'To study the defect of hypermetropia, to locate the image formed by a hypermetropic eye and to determine the power of the correcting lens.',
  unit: 'optics', chapter: 'Unit VI · Defects of Vision', kind: 'theory', difficulty: 'easy',
  tags: ['hypermetropia', 'defect of vision', 'convex lens', 'near point', 'eye']
};
