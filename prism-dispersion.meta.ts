// GENERATED FILE — do not edit by hand.
// Regenerate with: node scripts/gen-catalogue.mjs
import type { ExperimentMeta } from '@/experiments/registry';

export const meta: ExperimentMeta = {
  id: 'prism-dispersion', slug: 'prism-dispersion',
  title: 'Dispersion of Light Through a Prism',
  shortTitle: 'Prism Dispersion',
  aim: 'To determine the angle of minimum deviation for a given prism by plotting a graph between the angle of incidence and the angle of deviation.',
  unit: 'practical-b', chapter: 'Unit VI · Refraction and Dispersion', kind: 'practical', difficulty: 'moderate',
  practicalNo: 'B5', tags: ['prism', 'dispersion', 'minimum deviation', 'refractive index', 'spectrum']
};
